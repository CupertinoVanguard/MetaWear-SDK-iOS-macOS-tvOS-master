...
class DeviceViewController: UIViewController, UISearchBarDelegate, SendXYZAndDevice {
    ...
    */
    ...
    func moveSounds(data: dataForMovement){
        timer = Timer()
        //startTime = TimeInterval()
        let aSelector : Selector = #selector(self.moveSoundsLinearPath(data))
        timer = Timer.scheduledTimer(timeInterval: 0.01, target: self, selector: aSelector,     userInfo: nil, repeats: true)
        //startTime = Date.timeIntervalSinceReferenceDate
        //play seagulls here
        playSoundsController.play(index: data.Index)
    }
    
    func moveSoundsLinearPath(data : dataForMovement){
        
        playSoundsController.updatePosition(index: data.Index, position: data.Position)
        data.Position.x += data.moveX
        data.Position.y += data.moveY
        data.Position.z += data.moveZ
        if data.Position.x > data.XMax {
            playSoundsController.stop(index: data.Index)
            stopTimer()
            data.Position.y = data.XMin
        }
        if data.Position.y > data.YMax {
            playSoundsController.stop(index: data.Index)
            stopTimer()
            data.Position.y = data.YMin
        }
        if data.Position.z > data.ZMax {
            playSoundsController.stop(index: data.Index)
            stopTimer()
            data.Position.z = data.ZMin
        }
    }
